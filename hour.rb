...
for user in @users
  ...
  if user.last_connectionsscore <= (Time.now-(24*60*60))
    # calculate average_stream_word_score for top connections
  	@connections = Connection.find(:all, :conditions => ["user_id = ? and average_stream_word_score = ?", user.id, 0], :order_by => "average_word_score DESC, appearances DESC", :limit => 10)
  	for connection in @connections
  	  avgconwordscore = 0
  	...
  	@connections = Connection.find(:all, :conditions => ["user_id = ? and average_stream_word_score = ?", user.id, 0], :order_by => "average_stream_word_score DESC, average_word_score DESC, appearances DESC", :limit => 10)
  	for connection in @connections
  		connection.times_in_top = connection.times_in_top+1
  		connection.save
  	end # end loop through connections to add in top ten count
  end # end check if last word email sent was at least 24 hours ago

  
  
  ###### SEND TOP WORDS EMAIL ######
